...
//*********************************************
  ...
  function FOnClickStyleDrawing(e)
  {
      ...
      {
          case 'CkOrizzontale'  : FModalita = TRK_LINE_HORIZONTAL; break;
          case 'CkVerticale'    : FModalita = TRK_LINE_VERTICAL; break;
          case 'CkNormale'      : FModalita = TRK_LINE_NORMAL; break;
          case 'AggiungiAncora' : FModalita = ADDANCORA; break;
          case 'CkErasePoint'   : FModalita = TRK_ERASE_POINT; FStartPoint = null; FEndPoint = null; FPaintCanvas(); break;
          default               : FModalita = TRK_LINE_HORIZONTAL; break;
      }
      ...
      else
      {
        ...
        if(AnswerZone != undefined)
        {
            ...
            for(TmpZona of ArrayZone)
            {
                ...
                if(SingolaZona[0] >= 1)
                   FLsZone[SingolaZona[0]] = {xLeft:SingolaZona[1],xRight:SingolaZona[2],yUp:SingolaZona[3],yDown:SingolaZona[4]};
                SingolaZona = null;
            }
            ArrayZone = null;
        }
        ...
        if(AnswerHistory != undefined)
        {
            ...
            for(TmpHistory of ArrayHistory)
            {
                SingolaHistory = TmpHistory.split(',');
                   FLsHistory.push({IconaEvento:SingolaHistory[0],Tag:SingolaHistory[1],Ancora:SingolaHistory[2],TimeStamp:SingolaHistory[3],xPos:SingolaHistory[4],yPos:SingolaHistory[5],DescEvt:SingolaHistory[6]});
                SingolaHistory = null;
  ...
  function FLoadAncore(AnswerAncore)
  {
        ...
        if(AnswerAncore != undefined)
        {
            ...
            for(TmpAncora of ArrayAncore)
            {
                ...
                {
                   FLsAncore[SingolaAncora[0]] = {xPos:SingolaAncora[1],
  ...
  function FLoadVideocamere(AnswerVideocamere)
  {
        ...
        if(AnswerVideocamere != undefined)
        {
            ...
            for(TmpVideocamera of ArrayVideocamere)
            {
                ...
                FLsVideocamere[SingolaVideocamera[0]] = {xPos      : SingolaVideocamera[1],
  ...
  function FSalvaCopiaPaginaZone()
  {
      ...
      for(var i = 1; i <= NR_RIGHE_ZONE; i++)
      {
          ...
          FCopiaZone[i + PrimaRiga - 1] = {xLeft:document.getElementById('zonXLeft' + i).value,
  ...
  function FChangeStato(NewStato)
  {
    ...
    if(NewStato == STATO_VISUALIZZAZIONE)
      FTimer = setInterval(function() {FConnessione.send('GETBEACONS~' + FPianoAttuale);
                                       FConnessione.send("GETANCHORS");}, 1000);
  ...
  function mouseMove(event)
  {
    ...
    switch(event.type)
    {
       case "mousedown"       : event.preventDefault();
                                ...
                                break;
       case "mouseup"         : mouse.buttonRaw &= mouse.buttons[event.which + 2];
                                break;
       case "mouseout"        : mouse.buttonRaw = 0;
                                mouse.over = false;
                                break;
       case "mouseover"       : mouse.over = true;
                                break;
       case "mousewheel"      : event.preventDefault()
                                mouse.w = event.wheelDelta;
                                break;
       case "DOMMouseScroll"  : mouse.w = -event.detail;
                                break;
    }
        ...
        if(FGoogleMapMode)
        {
          ...
          if(SomeChange)
             if((FGoogleClusterMarker == null) && (FGoogleMarkers.length != 0))
             {
                ...
                FGoogleClusterMarker = new MarkerClusterer(GlobalGoogleMap,TmpMarkers,{imagePath: "MarkerClusterer/m",imagePathAlarm: "MarkerClusterer/mA"});
             }
        }
            ...
            if(FLsFari[i] != undefined)
            {
              if((FLsFari[i].xPos / 100 <= FLarghezza) &&
                 ...
                 CanDrawFaroOnCanvas(FLsFari[i]))
                 {
                  ...
                  var Riassunto = { Elmetto             : FLsFari[i].Elmetto,
  ...
  function FOnClickCanvas(e)
  {
    ...
    function GetPuntoVicino(PuntoVicino)
    {
       ...
       {
           case 1: return({X:FLsSegmenti[PuntoVicino.index].X1,Y:FLsSegmenti[PuntoVicino.index].Y1});
           case 2: return({X:FLsSegmenti[PuntoVicino.index].X2,Y:FLsSegmenti[PuntoVicino.index].Y2});
       }
       return(null);
    }
    ...
    if(FStato != STATO_EDITING)
    {
        ...
        if(FoundElemento) ShowMessage(function() { return(ElencoAllarmi);},
                                      SetAlarmButton);
      ...
      if((RealPosX >= 0) && (RealPosY >= 0))
      {
          ...
          else
          {
              ...
              else
              {
                  ...
                  FLsSegmenti.push({X1:FStartPoint.X,Y1:FStartPoint.Y,X2:FEndPoint.X,Y2:FEndPoint.Y,ColorLine1:FColorLine,ColorLine2:FColorLine});
                  FEndPoint = null;
                  FStartPoint = null;
                  FPaintCanvas();
              }
  ...
  function FTranslatePunti(Larghezza,Tacca,Punti,nome_piano,img_mappa)
  {
      ...
      try { FLarghezza = Larghezza / 100;} catch(E) { FLarghezza = 10; }
      if (!FLarghezza) FLarghezza = 10;
      try { FTacca = Tacca / 100;} catch(E) { FTacca = 1; }
      if (!FTacca) FTacca = 1;
      ...
      if(Punti != undefined)
      {
          ...
          for(TmpSegment of ArraySegmenti)
          {
              ...
              FLsSegmenti.push({X1:ArrayPunti[0] / 10,Y1:ArrayPunti[1] / 10,X2:ArrayPunti[2] / 10,Y2:ArrayPunti[3] / 10,ColorLine1:FColorLine,ColorLine2:FColorLine});
              ArrayPunti = null;
          }
          ArraySegmenti = null;
      }
    ...
    if ((RealPosX >= 0) && (RealPosY >= 0))
        {
            ...
            if(FStartPoint != null)
            {
                ...
                switch(FModalita)
                {
                   case TRK_LINE_HORIZONTAL : FEndPoint = {X:RealPosX,Y:FStartPoint.Y}; break;
                   case TRK_LINE_NORMAL     : FEndPoint = {X:RealPosX,Y:RealPosY}; break;
                   case TRK_LINE_VERTICAL : FEndPoint = {X:FStartPoint.X,Y:RealPosY}; break;
            ...
            {
                if(Punto.ColorLine1 != FColorLine) {Punto.ColorLine1 = FColorLine; ForcePaint = true;}
                if(Punto.ColorLine2 != FColorLine) {Punto.ColorLine2 = FColorLine; ForcePaint = true;}
            }
            ...
            if(PuntoVicino != null)
            {
                ...
                switch(PuntoVicino.Punto)
                {
                    case 1: FLsSegmenti[PuntoVicino.index].ColorLine1 = ColorEvidence; break;
                    case 2: FLsSegmenti[PuntoVicino.index].ColorLine2 = ColorEvidence; break;
                }
                ForcePaint = true;
            }
  ...
  document.getElementById("BtnCloseMessage").addEventListener("click", function () { FChangeStato(FPreviousStato); });
  ...
  document.getElementById('BtnCambiaPassword').addEventListener('click',function () { FChangeStato(STATO_CAMBIO_PASSWORD);});
  ...
  document.getElementById('BtnShowMsgChiudi').addEventListener('click',function() { FChangeStato(STATO_VISUALIZZAZIONE);});
  document.getElementById('BtnAnnullaPassword').addEventListener('click',function () { FChangeStato(STATO_VISUALIZZAZIONE);});
  document.getElementById('BtnConfermaPassword').addEventListener('click',function () { FConnessione.send('CHGPASSWORD~' +  document.getElementById('pwdOldPassword').value + '~' + document.getElementById('pwdNuovaPassword').value); });
  ...
  document.getElementById('BtnStorico').addEventListener("click", function () { FChangeStato(STATO_STORICO);RefreshDivStorico();});
  document.getElementById('BtnConfermaStorico').addEventListener('click',function () {FChangeStato(STATO_VISUALIZZAZIONE)});
  ...
  document.getElementById("BtnAnnullaModifiche").addEventListener("click", function () {FModalita=TRK_LINE_HORIZONTAL; FStartPoint = null; FConnessione.send('GETPOINTS~' + FPianoAttuale);document.getElementById("DescrizioneIdAncora").style.display="none";document.getElementById("NomeAncora").style.display="hidden";});
  document.getElementById("BtnPosizioneAncore").addEventListener("click", function () { FChangeStato(STATO_MODIFICA_ANCORE);});
  document.getElementById('BtnAnnullaAncore').addEventListener('click',function () { FChangeStato(STATO_VISUALIZZAZIONE); FPaintCanvas();});
  document.getElementById('BtnSelezionaPiano').addEventListener('click',function () { FChangeStato(STATO_SELEZIONA_PIANO); FPaintCanvas();});
  document.getElementById('BtnConfermaAncore').addEventListener('click',FSetAncore);
  document.getElementById("BtnDimensioniMappa").addEventListener("click", function () { FChangeStato(STATO_CAMBIO_DIM);});
  document.getElementById('BtnAnnullaDimMappa').addEventListener('click',function () { FChangeStato(STATO_VISUALIZZAZIONE); });
  ...
  document.getElementById('BtnAnnullaPianoSelezionato').addEventListener('click',function () { FChangeStato(STATO_VISUALIZZAZIONE); FPaintCanvas();});
  document.getElementById('BtnConfermaPianoSelezionato').addEventListener('click',FOnClickBtnConfermaPianoSelezionato);
  document.getElementById('BtnDoveSiTrova').addEventListener('click',function () { FChangeStato(STATO_TROVA_BEACON); FPaintCanvas();});
  document.getElementById('BtnAnagraficaTag').addEventListener('click',function () { FChangeStato(STATO_MODIFICA_TAG); FPaintCanvas();});
  document.getElementById('BtnAnnullaFaroDaTrovare').addEventListener('click',function () { FChangeStato(STATO_VISUALIZZAZIONE); FPaintCanvas();});
  document.getElementById('BtnConfermaFaroDaTrovare').addEventListener('click',FOnClickConfermaFaroDaTrovare);
  document.getElementById("BtnConfermaAnagraficaTag").addEventListener("click", function () {FIdTag=document.getElementById("FaroSelezionato2").value;FNomeTag=document.getElementById("nomeTag").value; FSetNomeTag()});
  document.getElementById('BtnAnnullaAnagraficaTag').addEventListener('click',function () { FChangeStato(STATO_VISUALIZZAZIONE); FPaintCanvas();});
  document.getElementById('NumeroAncore').addEventListener('input',function() {FSalvaCopiaPaginaAncore(); FDisponiPaginaAncore()});
  document.getElementById('BtnPagAncorePrecedenti').addEventListener('click',function() {FSalvaCopiaPaginaAncore(); FPaginaAncore--;  FDisponiPaginaAncore();});
  document.getElementById('BtnPagAncoreSuccessive').addEventListener('click',function() {FSalvaCopiaPaginaAncore(); FPaginaAncore++;  FDisponiPaginaAncore();});
  //document.getElementById("BtnPosizioneZone").addEventListener("click", function () { FChangeStato(STATO_MODIFICA_ZONE);});
  document.getElementById('BtnAnnullaZone').addEventListener('click',function () { FChangeStato(STATO_VISUALIZZAZIONE); });
  document.getElementById('NumeroZone').addEventListener('input',function() {FSalvaCopiaPaginaZone(); FDisponiPaginaZone(); });
  document.getElementById('BtnPagZonePrecedenti').addEventListener('click',function() {FSalvaCopiaPaginaZone(); FPaginaZone--;  FDisponiPaginaZone();});
  document.getElementById('BtnPagZoneSuccessive').addEventListener('click',function() {FSalvaCopiaPaginaZone(); FPaginaZone++;  FDisponiPaginaZone();});
  ...
  function FHandleServerError(CodeError,Error)
  {
      switch(CodeError)
       {
           case '1': FShowError('SQL ERROR [' + Error + ']',false);
                     break;
           case '2': FShowError('Account sconosciuto',true);
                     break;
           case '3': FShowError('Password errata',true);
                     break;
           case '4': FShowError('Utente non loggato',false);
                     break;
           case '5': FShowError('System error [' + Error + ']',false);
                     break;
           default : FShowError('Errore sconosciuto',false);
                     break;
       }
  }
  ...
  FConnessione.onmessage = function(e)
                           {
                             ...
                             switch(Parametri[0])
                             {
                                 case 'LOGIN':             if(Parametri[1] == '0')
                                                           ...
                                                           break;
                                 case 'CHGPASSWORD':       if(Parametri[1] == '0')
                                                           {
                                                               FChangeStato(STATO_VISUALIZZAZIONE);
                                                           ...
                                                           break;
                                 case 'SETPOINTS':         if(Parametri[1] == '0')
                                                           {
                                                               FChangeStato(STATO_VISUALIZZAZIONE);
                                                           }
                                                           else FHandleServerError(Parametri[1],Parametri[2]);
                                                           break;
                                 case 'SETDIMMAP':         if(Parametri[1] == '0')
                                                           {
                                                               FShowError('Dimensioni mappa cambiate',true);
                                                               try { FLarghezza = document.getElementById('dimLarghezza').value / 100;} catch(E) { FLarghezza = 10; }
                                                               if (FLarghezza == 0) FLarghezza = 10;
                                                               try { FTacca = document.getElementById('dimTacca').value / 100;} catch(E) { FTacca = 1; }
                                                               FNomePiano=document.getElementById("NamePiani").value;
                                                           ...
                                                           break;
                                 case 'GETPOINTS':         if(Parametri[1] == '0')
                                                           {
                                                           ...
                                                           break;
                                 case 'GETHISTORY':         if(Parametri[1] == '0')
                                                           {
                                                               FLoadHistory(Parametri[2],Parametri[3],Parametri[4],Parametri[5]);
                                                           }
                                                           else FHandleServerError(Parametri[1],Parametri[2]);
                                                           break;
                                 case 'GETANCHORS':        if(Parametri[1] == '0')
                                                           {
                                                           ...
                                                           break;
                                 case 'GETVIDEO':        if(Parametri[1] == '0')
                                                           {
                                                           ...
                                                           break;
                                 case 'GETZONES':          if(Parametri[1] == '0')
                                                           {
                                                             FLoadZones(Parametri[2]);
                                                           }
                                                           else  FHandleServerError(Parametri[1],Parametri[2]);
                                                           break;
                                 case 'GETEVENTLIST':     if(Parametri[1] == '0')
                                                           {
                                 ...
                                 case 'GETFLOORS':        {
                                                          ...
                                                          break;
                                 case 'GETBEACONS':       if(FStato == STATO_VISUALIZZAZIONE)
                                                          {
                                                              if(Parametri[1] == '0')
                                                               {
                                                           ...
                                                           break;
                                 case 'SETANCHORS':        if(Parametri[1] == '0')
                                                           {
                                                           ...
                                                           break;
                                 case 'SETBEACONS':        if(Parametri[1] == '0')
                                                           {
                                                           ...
                                                           break;
                                 case 'SETZONES':         if(Parametri[1] == '0')
                                                           {
                                                           ...
                                                           break;
                                 case 'LOGOUT':            FPianoAttuale = null;
                                                           FSuperUser = false;
                                                           ShowAllButtons();
                                                           FChangeStato(STATO_LOGIN);
                                                           break;
                                 case 'ADVICE_TO_TAG':     if(Parametri[1] == '0')
                                                           {
                                                           ...
                                                           break;    
                                 default: FShowError(e.data,false); break;
                             }
                             Parametri = null;
                           };
